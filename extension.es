...
class AFKExtension {
    ...
    start(autoLaunch = false) {
        ...
        this.tab.innerHTML = `<style>.afk_settings input{width: calc(100% - 50px); margin-left: 50px;}</style><h3>AFK Messages</h3><p>Players can now add messages when they go AFK by saying /afk &lt;message&gt;. Once they say something in chat again or leave the server, their message will be removed. SERVER's AFK message can only be removed with /afkclear. AFK messages will not be preserved between bot launches.</p><p><strong>Important:</strong> Server can not trigger AFK messages.</p><h3>Commands</h3><ul> <li>/afk &lt;message&gt; - Use this command to set an AFK message.</li><li>/afkclear &lt;name&gt; (Admin only) - Manually clear the AFK message of a player.</li></ul><h3>Responses</h3><div class="afk_settings"> <label>When a message is set: <input data-setting="onSet"/></label> <label>When a message is cleared with the /afkclear command: <input data-setting="onClear"/></label> <label>When someone's name who is afk is mentioned: <input data-setting="onTrigger"/></label></div>`;
